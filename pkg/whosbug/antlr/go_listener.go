...
func (s *GoTreeShapeListener) EnterMethodDecl(ctx *golang.MethodDeclContext) {
	...
	if temp := getRecvrTypes(ctx); len(temp) > 0 {
		methodInfo = MethodInfoType{
			StartLine:  ctx.GetStart().GetLine(),
			EndLine:    ctx.GetStop().GetLine(),
			MethodName: temp[0] + "." + ctx.IDENTIFIER().GetText(),
			Parameters: getFunctionAndMethodParams(ctx),
		}
		s.AstInfoList.Methods = append(s.AstInfoList.Methods, methodInfo)
	}
...
func (s *GoTreeShapeListener) EnterTypeDecl(ctx *golang.TypeDeclContext) {
	var structInfo = ClassInfoType{
		ClassName: ctx.TypeSpec(0).(*golang.TypeSpecContext).IDENTIFIER().GetText(),
		StartLine: ctx.GetStart().GetLine(),
		EndLine:   ctx.GetStop().GetLine(),
	}
	s.AstInfoList.Classes = append(s.AstInfoList.Classes, structInfo)
}
